...
TODO: Stefano to 
    simplify below
    change `file` to `sample`
    change `curated_cell_type` to `cell_type`
    change to real categories instead of dummy

```{r}
pseudo_bulk <-
  sce_obj |>
  bioc2022tidytranscriptomics::aggregate_cells(c(file, curated_cell_type), assays = "counts") |>
...
The output is a nested table containing a SummarizedExperiment for each cell type.

```{r}
pseudo_bulk
```

We can use tidyverse `slice` to choose a row (cell_type) and `pull` to extract the values from a column. If we pull the data column we can view the SummarizedExperiment object. 
...
TODO: Stefano to see if can 
    use more realistic condition and filter (FDR)
    remove the filter map_int step
